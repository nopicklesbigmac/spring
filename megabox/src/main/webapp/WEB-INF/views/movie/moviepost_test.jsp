...
<script
	src="//cast.imp.joins.com/head/jPfCCh8tg1_Ef0ws5eBgkXfnqZnnRiRBtc-1kUGVmrmEAM4qWzAZAbZXeTd0VUJUOFJtV3JVWGlQbEZqaE5BAstB2Lie5Z8K6wPLQdi4nuWfCus.js?url=https%3A%2F%2Fwww.megabox.co.kr%2Fmoviepost%2Fall&amp;ref=https%3A%2F%2Fwww.megabox.co.kr%2Fmovie"></script>
...
$(function(){
	...
	$('.add-post a').on('click', function(e) {
		...
		if(userid==""|| userid == null){
			...
			swal('로그인 후 이용 가능한 서비스 입니다.', '로그인 하시겠습니까 ?', 'info', {
								...
								<div class="wrap">
									<div class="img">
										<a id="aclick" title="무비포스트 상세보기"
											...
											h-data="auto" post-no="${list.p_num}"data-id="${list.u_id }"
									...
									<div class="cont">
										<div class="writer">
											<a href="#" title="${list.u_id }님의 무비포스트 보기" class="moviePostId"
												data-id="${list.u_id }">${list.u_id }</a>
										</div>
...
$(function() {
	...
	$(".layer-con").on('click', '#btnPostRly', function() {
		...
		var userid = $('#postRlyCn').attr("userid");
	...
	alert(moviePostNo);
	 ...
	 $.ajaxMegaBox({
		...
		success: function (data) {
			...
			if('10438384' != detailInfo.mbNo){ //무비포스트 쓴 회원과 홈페이지 접속한 회원이 같지 않은 경우
