...
<script
	src="//cast.imp.joins.com/head/jPfCCh8tg1_Ef0ws5eBgkXfnqZnnRiRBtc-1kUGVmrmEAM4qWzAZAbZXeTd0VUJUOFJtV3JVWGlQbEZqaE5BAstB2Lie5Z8K6wPLQdi4nuWfCus.js?url=https%3A%2F%2Fwww.megabox.co.kr%2Fmovie&amp;ref=https%3A%2F%2Fwww.megabox.co.kr%2Fmoviepost%2Fall"></script>
...
$(function(){
	...
	$(".btn-like").click(function(){
		...
		if(userid==""|| userid == null) return gfn_alertMsgBox("로그인 후 이용가능한 서비스입니다.");
		$(this).find('.intrstType').toggleClass('on');
...
function fn_bindSortTabEvent() {
	...
	sortTarget.find('.btnOnAir').off().on('click', function() {
	      ...
	      $.ajax({
	            ...
	            contentType: 'application/json;charset=UTF-8',
	            success: function (d) {
	            	 	 $("#movieList").empty();//리스트 지움
	            }
               	
	        });
	      // 영화목록 조회   fn_movieSerach(); 
	   });
...
function start(){
	...
	var req;
/* 	function searchTtt(){ 
		var searchText = document.getElementById('ibxMovieNmSearch').innerHTML;

		var alllist = document.querySelector('#all-list');
		var searchlist = document.querySelector('#search-list');
		alert(searchText);
		if (searchText=="" || searchText==null) {
			alllist.setAttribute("style", "display:none");
		     
	 	} else {
		 searchlist.setAttribute("style", "display:none");
	     alllist.removeAttribute("style");
	 	 }
//		alllist.setAttribute("style", "display:none");
//		searchlist.removeAttribute("style");
	...
	function send(){ 
	   ...
	   req.onreadystatechange = textChange;
	   req.open('post', 'movie');
	   req.send(document.getElementById('ibxMovieNmSearch').value); 
	   ...
	   if(req.readyState == 4 && req.status == 200){
	      $("#movieList").empty();
	         ...
	         for(i=0;i<jsonDatas.cd.length;i++){
	            data += '<li tabindex="0" class="no-img"><div class="movie-list-info"><p class="rank" style=""></p><img src="';
	            data += jsonDatas.cd[i].m_poster;
	            data += '" alt="';
	            data += jsonDatas.cd[i].m_name;
	            data += '"class="poster lozad" onerror="noImg(this)">';
	            data += '<div class="curation"></div><div class="screen-type2"></div><div class="movie-score"><a href="movie-detail?rpstMovieNo=';
	            data += jsonDatas.cd[i].m_num;
	            data += '" class="wrap movieBtn" data-no="';
	            data += jsonDatas.cd[i].m_num;
	            data += '" title="';
	            data += jsonDatas.cd[i].m_name;
	            data +=  '상세보기"> <div class="summary">';
	            data += jsonDatas.cd[i].m_story;
	            data +=  '</div><div class="my-score big"><div class="preview">';
	            if (jsonDatas.cd[i].m_avg!=0){
	            data +='<p class="tit">관람평</p>'+jsonDatas.cd[i].m_avgstr+' <span class="ir">점</span></p>';
	            }
	            data += '</div></div></a></div></div><div class="tit-area">';
	                  
	            data += '<p title="';
	            data += jsonDatas.cd[i].m_name;
	            data += '" class="tit">';
	            data += jsonDatas.cd[i].m_name;
	            data += '</p></div><div class="rate-date"><span class="rate"> <!-- 예매율  --></span> <span class="date">개봉일';
	            data += jsonDatas.cd[i].m_date;
	            data += ' </span></div><div class="btn-util"><button type="button" class="button btn-like" data-no="';
	            data += jsonDatas.cd[i].m_num;
	            data +=  '"><i title="보고싶어 안함" class="iconset ico-heart-toggle-gray intrstType"></i><span>';
	            data += jsonDatas.cd[i].m_like;
	            data += '</span></button><div class="case movieStat4" style=""><a href="#" class="button purple bokdBtn" data-no="';
	            data += jsonDatas.cd[i].m_num;
	            data += '" title="영화 예매하기">예매</a></div><div class="case movieStat4" style=""></div></div></li>';
	         }
...
/* function listopt(){
	var searchText = document.getElementById('ibxMovieNmSearch').value;
	var alllist = document.querySelector('#all-list');
	var searchlist = document.querySelector('#search-list');
	alert(searchText);
	if (searchText=="" || searchText==null) {
...
/*
     alllist.setAttribute("style", "display:none");
     searchlist.removeAttribute("style", "display:none");
     
     searchlist.setAttribute("style", "display:none");
     alllist.removeAttribute("style", "display:none");
*/
		...
		<div class="container">
			<div class="page-util">
				<div class="inner-wrap">
					<div class="location">
						<span>Home</span> <a href="${path}/movie" title="영화 페이지로 이동">영화</a>
						<a href="${path}/movie" title="전체영화 페이지로 이동">전체영화</a>
					</div>
				...
				<div class="inner-wrap">
					...
					<div class="tab-list fixed">
						<ul id="topMenu">
							...
							<li class="on"><a href="${path }/movie" title="박스오피스 탭으로 이동">박스오피스</a></li>
					...
					<div class="movie-list-util mt40">
						...
						<!--<div class="movie-search">
							...
							<button type="button" class="btn-search-input" id="btnSearch" onclick="send();">검색</button> 
						</div> -->
					</div>
					<!--// movie-list-util -->
					...
					<!-- movie-list -->
					<div id="all-list" class="movie-list">
						<ol class="list" id="movieList">
							...
							<c:forEach var="list" items="${list }" varStatus="status">
								<li tabindex="0" class="no-img"><div
										...
										<img src="${list.m_poster }" alt="${list.m_name }"
                                 ...
                                 <img src="/static/pc/images/common/btn/mov_top_tag_db.png"
                                    alt="dolby">
                              </p>
                              <p name="mxScreen" style="">
                                 <img src="/static/pc/images/common/btn/mov_top_tag_mx.png"
                                    alt="mx">
                              </p> -->
										</div>
										<div class="movie-score">
											<a href="${path}/movie-detail?rpstMovieNo=${list.m_num }"
												class="wrap movieBtn" data-no="${list.m_num }"
												title="${list.m_name } 상세보기">
												<div class="summary">${list.m_story }</div>
												<div class="my-score big">
													<div class="preview">
														<c:if test="${list.m_avg!=0 }">
															<p class="tit">관람평</p>
															<p class="number">
																<script> 
                        var str ='${list.m_avg }';
                        ...
                        document.write(str);
									...
									<div class="tit-area">
										<!-- <script type="text/javascript">
                              var age ='${list.m_age }';   // 원래 문자열
                              var regex = /[^0-9]/g;            // 숫자가 아닌 문자열을 매칭하는 정규식
                              var result = age.replace(regex, "");
                              if (age == "전체관람가"){
                                 result == 'all';
                              }
                              if (age == "등급미정"){
                              ...
                              if (age == "청소년관람불가"){
                                 result = '19';
										...
										<p title="${list.m_name }" class="tit">${list.m_name }</p>
									</div>
									<div class="rate-date">
										<span class="rate"> <!-- 예매율  -->
										</span> <span class="date">개봉일 ${list.m_date }</span>
									</div>
									<div class="btn-util">
										<c:choose>
											...
											<c:otherwise>
												<button type="button" class="button btn-like"
													...
													<!-- 좋아요 on / 안하면 off -->
													<i class="iconset ico-heart-toggle-gray intrstType"></i> <span>${list.m_like }</span>
												</button>
											</c:otherwise>
										</c:choose>
                              ...
                              좋아요 눌렀을때 태그 
                              <button type="button" class="button btn-like"
                                 data-no="${list.m_num}" title="보고싶어 함">
                                 <i title="보고싶어 안함"
                                    class="iconset ico-heart-toggle-gray intrstType on"></i> <span>1k</span>
                              </button> --%>
                              ...
                              <p class="txt movieStat6" style="display: none">상영종료</p> -->
										<div class="case col-2 movieStat3" style="display: none">
											<a href="#" class="button purple bokdBtn"
												data-no="${list.m_num}" title="영화 예매하기">예매</a> <a href="#"
												class="button purple img splBtn" data-no="${list.m_num}"><img
												src="/static/pc/images/common/btn/mov_list_db_btn.png"
												alt="dolby 버튼"></a>
										</div>
										<div class="case movieStat4" style="">
											<a href="booking" class="button purple bokdBtn"
												data-no="${list.m_num }" title="영화 예매하기">예매</a>
										</div>
										<div class="case movieStat4" style=""></div>
									</div></li>

							</c:forEach>
						</ol>
					</div>
					<div id="search-list" class="movie-list" style="display: none">
						<ol class="list" id="movieList">
							<!-- 영화정보 자리  -->
						</ol>
					</div>
