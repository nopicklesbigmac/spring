...
<script
	src="//cast.imp.joins.com/head/jPfCCh8tg1_Ef0ws5eBgkXfnqZnnRiRBtc-1kUGVmrmEAM4qWzAZAbZXeTd0VUJUOFJtV3JVWGlQbEZqaE5BAstB2Lie5Z8K6wPLQdi4nuWfCus.js?url=https%3A%2F%2Fwww.megabox.co.kr%2Fmoviepost%2Fwrite&amp;ref=https%3A%2F%2Fwww.megabox.co.kr%2Fmoviepost%2Fall"></script>
...
var updateData = {
    ...
    movieNoRq : '',
    movieNmRq : '',
   	imgSvrUrl : 'https://img.megabox.co.kr',
   	backScreen: ''
}

$(function(){
    // 내가 본 영화 등록 클릭
    $('.myViewMovieBtn').on('click', function(e) {
        e.preventDefault();

        gfn_alertMsgBox('준비중입니다.');
    });


    // 내가 본 영화 더보기 클릭
    $('#addMovieDiv').on('click', function(e) {
        e.preventDefault();
    ...
    // 영화 선택 버튼
    $('#selectMovieBtn').on('click', function(e) {
        e.preventDefault();

        if($('#movieList .on').length > 0) {
            var movieNo = $('#movieList .on').data('no');
            var movieNm = $('#movieList .on').data('nm');
            $('input[name=movienum]').attr('value',movieNo);
            fn_initStep2(movieNo, movieNm);
        } else {
            gfn_alertMsgBox('영화를 선택해주세요.');
        }
    });

    // 영화 선택 취소 버튼
    $('#cacelMovieBtn').on('click', function(e) {
        e.preventDefault();

        if($('.selectMovie').css('display') != 'none') {
            $('#movieList').empty();
        }
    });

    // 영화 다시선택 클릭
    $('#resetMovieBtn').on('click', function(e) {
        e.preventDefault();
    ...
    // 스틸컷, 예고편, 내사진 탭 클릭
    $('#writeTab .btn').on('click', function() {
        $('#writeTab .btn.on').removeClass('on');
        $(this).addClass('on');

        $('.postContents').hide();
        $('#' + $(this).attr('write-type')).show();
    });

    // 내 컴퓨터 사진 찾기 버튼
    $('#myphotoAddBtn').click(function(){
          fn_fileUploadForm();
    });

    // 무비포스트 레이아웃 선택
    $('.layout-list a').on('click', function() {
        $('#postLayer .select.on .post-layer').attr('class', 'post-layer').addClass('layer-type-' + $(this).attr('layer-type'));

        $('.layout-list').find('li').removeClass('on');
        $(this).parents('li').addClass('on');
    });


    // 무비포스트 취소 클릭
    $('#postCancelBtn').on('click', function(e) {
        e.preventDefault();
        fn_moveListPage();
    });
    ...
    $('#regBtn').on('click', function(e) {
        ...
        $.each($('#postLayer .select'), function(i, v) {
        	
        	if(fileName == undefined) {
                $('.select').eq(i).trigger('click');
                result = false;
                
                return gfn_alertMsgBox('이미지를 선택해주세요');
            } else if(gfn_isEmpty($(v).find('.post-txt textarea').val())) {
                $('.select').eq(i).trigger('click');
                result = false;
                return gfn_alertMsgBox('내용을 입력해주세요');
            } else {
                var target = $(v).find('.post-bg');
                var obj = {
                    moviePostImgNo: target.data('no'),
                    moviePostImgDesc: $(v).find('.post-txt textarea').val(),
                    atchFileUrl: target.data('url'),
                    moviePostImgDivCd: target.data('cd'),
                    imgPosiVal: $(v).find('.post-layer').attr('class').split('layer-type-')[1]
                }
                imgList.push(obj);
            }
        });

        var btnTxt = $.trim($('#regBtn').text());

        if(!result) return;
        if(!confirm(btnTxt + ' 하시겠습니까?')) return;

        var param = {
            dlParamData: {
                rpstMovieNo: $('#resetMovieBtn').data('no'),
                movieNm: $('#resetMovieBtn').data('nm'),
                moviePostNo: $('#moviePostNo').val(),
                mergeType: $('#mergeType').val(),
                imgList: imgList
            }
        };

        $.ajaxMegaBox({
            url: '/on/oh/oha/Movie/mergeMoviePost.do',
            ...
            success: function (d) {
                ...
                if(resultMap.msg == 'already') return gfn_alertMsgBox('이미 작성된 포스트가 있습니다.');

                var options = {};
                options.msg = btnTxt + ' 되었습니다.';
                options.callback  = fn_moveListPage;
    ...
    if(updateData.moviePostNo) {
        fn_initStep2(updateData.movieNo, updateData.movieNm, true);
    } else if(updateData.movieNoRq) {
        fn_initStep2(updateData.movieNoRq, updateData.movieNmRq)
    } else {
        // 페이지 초기화 함수
        fn_initStep1();
    }
...
// 무비포스트 목록 페이지 이동
function fn_moveListPage() {

    var s_movieNo = "";

    if (updateData.movieNoRq != null && updateData.movieNoRq != '') {
    	s_movieNo = updateData.movieNoRq;
    } else {
    	s_movieNo = updateData.movieNo;
    }

    if (updateData.backScreen != null && updateData.backScreen == "movie-detail") {
        $('#mPostForm').append('<input type="hidden" name="rpstMovieNo" value="' + s_movieNo + '">');
        $('#mPostForm').attr('action', '/movie-detail');
    } else if (updateData.backScreen != null && updateData.backScreen == "movie-detail/moviepost") {
            $('#mPostForm').append('<input type="hidden" name="rpstMovieNo" value="' + s_movieNo + '">');
            $('#mPostForm').attr('action', '/movie-detail/moviepost');
    } else if (updateData.backScreen != null && updateData.backScreen == "movie-timeline") {
        $('#mPostForm').attr('action', '/mypage/moviestory');
    } else if (updateData.backScreen != null && updateData.backScreen == "my-moviePost") {
        $('#mPostForm').append('<input type="hidden" name="cd" value="mp">');
        $('#mPostForm').attr('action', '/mypage/moviestory');
    } else if (updateData.backScreen != null && updateData.backScreen == "my-seenMovie") {
        $('#mPostForm').append('<input type="hidden" name="cd" value="sm">');
        $('#mPostForm').attr('action', '/mypage/moviestory');
    } else {
        $('#mPostForm').attr('action', '/moviepost/all');
    }

    $('#mPostForm').attr('method', 'post');
    $('#mPostForm').submit();
...
// 무비포스트 스텝1 초기화
function fn_initStep1() {
    $('.movieList').show();
    $('.selectMovie').hide();
    /* $('#movieList').empty(); */
//    $('#postLayer .select').remove();
    fn_addPost();
    $("#currentPage").val("1");
    // 내가 본 영화 목록 조회
    fn_movieSerach(true);
...
// 무비포스트 스텝2 초기화
function fn_initStep2(movieNo, movieNm, updateYn) {
    $('.movieList').hide();
    $('.selectMovie').show();
    $('#movieNmTit').text(movieNm);


    if(updateYn == 'Y') { // 무비포스트 레이아웃 초기화 - 수정이 아닐시
        $('#postLayer .select').remove();
        fn_addPost();
    } else { // 수정시 무비포스트 레이아웃 이벤트
        fn_moviePostMouseEvent();
    }

    $('#resetMovieBtn').attr('data-no', movieNo);
    $('#resetMovieBtn').attr('data-nm', movieNm);

    fn_imgSerach('still', movieNo); // 스틸컷 목록
    fn_imgSerach('trailer', movieNo); // 예고편 목록
}

// 무비포스트 레이아웃 마우스 이벤트
function fn_moviePostMouseEvent() {
    // 무비포스트 레이아웃 선택 이벤트
    ...
    // 무비포스트 레이아웃 삭제 이벤트
    $('#postLayer .cancle-select').off().on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();

        if($('#postLayer .select').length > 1) {
            $(this).parent('.post-layer').parent().remove();
        } else {
            gfn_alertMsgBox("1건 미만인 경우, 삭제할 수 없습니다.");
        }
    });

    // 무비포스트 텍스트아레아 이벤트
    $('.post-txt textarea').off().on('keyup', function() {
        if(this.value.length > 50) {
            gfn_alertMsgBox('최대 50자까지 입력 가능합니다.');

            this.value = this.value.substring(0, 50);
        }

        $(this).next().text(this.value.length + ' / 50');
    })
}

// 내가 본 영화목록 가져오기
function fn_movieSerach(resetType){
    var currentPage = $("#currentPage").val();
    var recordCountPerPage = $("#recordCountPerPage").val();
    var totalCnt, totMovieCnt;

    var paramData = { currentPage: currentPage + '', recordCountPerPage : recordCountPerPage + '', seenYn: 'Y' };

    $.ajaxMegaBox({
    	
        url: '/on/oh/oha/Movie/selectMovieLayerList.do',
        data: JSON.stringify(paramData),
        success: function (d) {
            var movieList = d.movieList;

            if(resetType) $('#movieList').empty();
            ...
            if(movieList) {
                ...
                $.each(movieList, function(i, v) {
                    ...
                    if(v.moviePostNo == '0') {
                        html += '    <a href="#" title="작성완료" class="done" data-no="' + v.rpstMovieNo + '" data-nm="' + v.movieNm + '">';
                    } else {
                        html += '    <a href="#" title="' + v.movieNm + ' 선택" data-no="' + v.rpstMovieNo + '" data-nm="' + v.movieNm + '">';
                    }

                    html += '        <div class="bg-chk"></div>';
                    html += '        <div class="bg-done">작성완료</div>';

                  //  if(v.moviePostRegYn == 'Y') {
                  //      html += '    <img src="' + d.imgSvrUrl + v.imgPathNm + '" onerror="noImg(this)" alt="' + v.movieNm + '">';
                  //  } else {
                        html += '    <img src="' + d.imgSvrUrl + nvl(v.imgPathNm).posterFormat('_150') + '" onerror="noImg(this)" alt="' + v.movieNm + '">';
                  //  }
                    ...
                    totalCnt = v.totCnt;
                    totMovieCnt = v.totMovieCnt;

                    $('#movieList').append(html);
                });

                // 페이징 버튼 설정
                if(!MegaboxUtil.Common.isPaging(currentPage, recordCountPerPage, totalCnt)) {
                    $('#addMovieDiv').hide();
                } else {
                    $('#addMovieDiv').show();
                }

                $('#totMovieCnt').text(totMovieCnt);
...
// 왼쪽 스틸컷, 예고편 검색
function fn_imgSerach(searchtype, movieNo){
    var url = "";
    var target = "";

    if(searchtype == "still"){
        url = "/on/oh/oha/Movie/selectStillList.do";
        target = "stillList";
    }else{//trailer
        url = "/on/oh/oha/Movie/selectTrailerList.do";
        target = "trailerList";
    }

    var paramData = { rpstMovieNo: movieNo, searchtype :searchtype, position: 'MoviePost' };

    $.ajaxMegaBox({
        url: url,
        data: JSON.stringify(paramData),
        success: function (data) {
            var ul, li, a, video, source, div, img;
            var targetTag = $("#" + target);
            var imgSvrUrl = data.imgSvrUrl;

            targetTag.find('ul').remove();

            ul = $('<ul>');
            if(searchtype == "still") {
                $.each(data.imgList, function(i, v) {
                    li = $('<li>');
                    div = $('<div>').attr('class', 'selected-line');
                    a = $('<a>').attr({ 'href': '#', 'class': 'stillBtn', 'still-img-path': imgSvrUrl + v.imgPath, 'atch-file-no': v.atchFileNo, 'movie-img-kind-cd': v.movieImgKindCd, 'movie-vod': '' });
                    img = $('<img>').attr({ 'src': imgSvrUrl + nvl(v.imgPath).posterFormat('_150'), 'alt': '' });
                ...
                $.each(data.videoList, function(i, v) {
                    li = $('<li>');
                    /*
                    a = $('<a>').attr({ 'href': '#', 'class': 'trailerBtn', 'trailer-img-path': imgSvrUrl + v.previewImgUrl, 'atch-file-no': v.atchFileNo, 'atch-file-url': v.atchFileUrl, 'movie-img-kind-cd': v.movieImgKindCd });
                    video = $('<video>').attr({ 'preload': 'none', 'poster': imgSvrUrl + v.previewImgUrl });
                    source = $('<source>').attr({ 'src': v.pcUrl });

                    a.append(video).append(source);
					*/
                    div = $('<div>').attr('class', 'selected-line');
                    a = $('<a>').attr({ 'href': '#', 'class': 'trailerBtn', 'trailer-img-path': imgSvrUrl + v.previewImgUrl, 'atch-file-no': v.atchFileNo, 'movie-img-kind-cd': v.movieImgKindCd, 'movie-vod': v.pcUrl  });
                    img = $('<img>').attr({ 'src': imgSvrUrl + nvl(v.previewImgUrl).posterFormat('_150'), 'alt': '' });
...
//스틸컷 버튼 이벤트
function fn_bindImgEvent() {
    $('.stillBtn').off();
    $('.stillBtn').on("click", function(e) {
        e.preventDefault();

        var atchFileNo = $(this).attr('atch-file-no');
        var imgPath = $(this).attr('still-img-path');
        var movieImgKindCd = $(this).attr('movie-img-kind-cd');
        var selectIdx = $('#postLayer .select.on').index();
        var target = $('.select').eq(selectIdx);
        var moviePostImgDivCd = '';
        var movieVod = $(this).attr('movie-vod');

        /* if(movieImgKindCd == 'MIK01') {
            moviePostImgDivCd = 'poster';
        } else if(movieImgKindCd == 'MIK02') {
            moviePostImgDivCd = 'still';
        } else if(movieImgKindCd == 'MIK03') {
            moviePostImgDivCd = 'trailer';
        } */

        $(".post-bg").eq(selectIdx).html("");
       // $(".post-bg").eq(0).html("");

        moviePostImgDivCd = movieImgKindCd;

        target.find('.post-bg').attr('data-no', atchFileNo);
        target.find('.post-bg').attr('data-url', '');   //imgPath);
        target.find('.post-bg').attr('data-cd', moviePostImgDivCd);

		$(".post-bg").eq(selectIdx).append("<img src='' alt=''/>");

		imgPath = movieImgKindCd == 'MIK01' ? nvl(imgPath).posterFormat('_316') : nvl(imgPath).posterFormat('_380');
        $('.post-bg').eq(selectIdx).find('img').attr('src', imgPath );
      //  $('.post-bg').eq(selectIdx).find('img').attr('src', imgPath );
        $('.post-layer').eq(selectIdx).addClass('layer-type-01');
    });
}

// 예고편 버튼 이벤트
function fn_bindVideoEvent() {
    $('.trailerBtn').off();
    $('.trailerBtn').on("click", function(e) {
        e.preventDefault();
		/*
        if($('#postLayer .select.on').index() == 0){
            return gfn_alertMsgBox('첫번째는 스틸컷만 선택 가능합니다.');
        }
		*/
        var selectIdx = $('#postLayer .select.on').index();
        var atchFileNo = $(this).attr('atch-file-no');
        var imgPath = $(this).attr('trailer-img-path');
        var atchFileUrl = $(this).attr('atch-file-url');
        var movieImgKindCd = $(this).attr('movie-img-kind-cd');
        var moviePostImgDivCd = '';
        var movieVod = $(this).attr('movie-vod');
        ...
        $("input[name='moviePostImgNo']").eq(selectIdx).val(atchFileNo);

        $(".post-bg").eq(selectIdx).html("");
        $("input[name='moviePostImgNo']").eq(selectIdx).val("");
        $("input[name='atchFileUrl']").eq(selectIdx).val("");
        $("input[name='moviePostImgDivCd']").eq(selectIdx).val("");

        $(".post-bg").eq(selectIdx).append("<video controls=\"controls\" preload=\"none\" poster=\""+nvl(imgPath).posterFormat('_380')+"\"><source src=\""+movieVod+"\"></video>");

        var target = $('.select').eq(selectIdx);
        target.find('.post-bg').attr('data-no', atchFileNo);
        target.find('.post-bg').attr('data-url', movieVod);
        target.find('.post-bg').attr('data-cd', moviePostImgDivCd);

      //  $("input[name='moviePostImgNo']").eq(selectIdx).val(atchFileNo);
      //  $("input[name='atchFileUrl']").eq(selectIdx).val(movieVod);
      //  $("input[name='moviePostImgDivCd']").eq(selectIdx).val(moviePostImgDivCd);
    });
}

// 무비포스트 레이어 추가
/* function fn_addPost() {
    var lastIndex = $('#rowCnt').val();

    $('#postBtnLayer').remove();
    $('#postLayer .select.on').removeClass('on');

    var htmlText = '';

    htmlText += '<div class="select on">';
    htmlText += '    <div class="post-layer add-post">';
    htmlText += '        <div class="post-bg">';
    htmlText += '            <img src="${path }/resources/img/bg-img-select.png" alt="이미지를 선택하세요!">';
    htmlText += '        </div>';
    htmlText += '        <div class="post-txt">';
    htmlText += '            <textarea name="" id="" title="내용을 작성해 주세요!" placeholder="내용을 작성해 주세요!" /></textarea>';
    htmlText += '            <div class="text-count">0 / 50</div>';
    htmlText += '        </div>';
    htmlText += '        <div class="cancle-select"><a href="#" title="닫기" class="layer-close"><i class="iconset ico-layer-close"></i></a></div>';
    htmlText += '    </div>';
    htmlText += '</div>';
  //  htmlText += '<div class=\"btn-group a-c pt30\" id=\"postBtnLayer\">';
  //  htmlText += '	<button type=\"button\" class=\"button dash-btn\" onclick=\"fn_addPost();\"><i class=\"iconset ico-\"></i> 포스트 추가</button>';
  //  htmlText += '</div>';

    $('#postLayer').append(htmlText);
    $('#rowCnt').val(Number(lastIndex) + 1);
...
/* var fn_fileUploadForm = function() {
    var options = {};
    options.maxSize = 2;    // 단위 2MB
    options.fileNo = '';
	options.fileExt = "JPG, JPEG, PNG, GIF";
	options.baseType = "webImg";
	options.uploadType = "poster";
 	options.resizeFileSize = "600";
 
    options.success = function(data, textStatus, jqXHR) {
    	alert('hi');
        var imgPathNm = data.dlFileInfoM.imgPathNm;
        var fileNo = data.dlFileInfoM.fileNo;
        var fileSn = data.dlFileInfoM.fileSn;
        var selectIdx = $('#postLayer .select.on').index();
...
//        $('.post-bg').eq(selectIdx).css('background-image', 'url("' + updateData.imgSvrUrl + imgPathNm + '")');
//        $('input[name="moviePostImgNo"]').eq(selectIdx).val(fileNo);
//        $('input[name="moviePostImgDivCd"]').eq(selectIdx).val(moviePostImgDivCd);
		
		document.getElementById(".post-bg img").src = "#";
		$(".post-bg").eq(selectIdx).append("<img src='' alt=''/>");
        $('.post-bg').eq(selectIdx).find('img').attr('src', updateData.imgSvrUrl + imgPathNm);



        var target = $('.select').eq(selectIdx);
        target.find('.post-bg').attr('data-no', fileNo);
        target.find('.post-bg').attr('data-url', '');
        target.find('.post-bg').attr('data-cd', 'MOVIEPOST');
    };
	options.fileSizeError = function(fileSize, maxSize) {
		gfn_alertMsgBox('사진은 2MB 이하만 첨부 가능합니다.');
	};
	options.fileExtError = function() {
		gfn_alertMsgBox('이미지 파일만 등록 가능합니다.');
	};
	...
	if(msg != "") {
		...
		<form id="mPostForm" method="post"  enctype="multipart/form-data"></form>
		
		<form id="moviePostFrm" name="moviePostFrm" action="${path}/writeProc" method="post" enctype="multipart/form-data">
<!-- 			<input type="hidden" id="currentPage" name="currentPage" value="1">
			<input type="hidden" id="recordCountPerPage" name="recordCountPerPage" value="14"> 
			<input type="hidden" id="imgPath" name="imgPath" value="">
			<input type="hidden" id="selectIdx" name="selectIdx" value="0"> 
			<input type="hidden" id="rowCnt" name="rowCnt" value="2"> 
			<input type="hidden" id="layerType" name="layerType" value="01">
			<input type="hidden" id="mergeType" name="mergeType" value="I">
			<input type="hidden" id="moviePostNo" name="moviePostNo" value="">
 -->
			<div class="container">
				<!-- page-tit-area -->
				<div class="page-util">
					...
					<div class="inner-wrap">
						...
						<div class="user-post-top movieList">
							...
							<div class="right">
								...
								등록가능한 영화 <strong class="font-gblue"><span
									id="totMovieCnt">${fn:length(list) }</span></strong> 건
							</div>
						</div>
						...
						<div class="movie-post-step01 movieList">
							<div class="movie-post">
								...
								<div class="select-post-list">
									...
									<!-- 전체영화목록 >> 본영화 목록  -->
									<c:if test="${list.m_num <= 14}">
									<li>
										<a href="#" title="${list.m_name }선택" data-no="${list.m_num }" data-nm="${list.m_name }">
												<div class="bg-chk"></div>
												 <img
												src="${list.m_poster }"
												onerror="noImg(this)" alt="${list.m_name }">
										</a></li>
										</c:if>
									</c:forEach>
                     ...
                     -->
							...
							<div class="btn-group mt20">
								<a href="moviepost" class="button large"
									id="cancelMovieBtn" title="취소">취소</a> <a href="#"
									class="button large purple" id="selectMovieBtn" title="등록">등록</a>
							</div>
						</div>

						<div class="movie-post-step02 selectMovie" style="display: none;">
							<div class="user-post-top">
								<p class="tit" id="movieNmTit"></p>
								<input type="text" name="movienum" id="movienum" value="" style="display: none;"/> 

								<div class="right">
									<a href="#" class="button" id="resetMovieBtn" title="다시선택"><i
										class="iconset ico-post-review"></i> 다시선택</a>
								</div>
							</div>
					
							<div class="post-layout-select">
								<div class="left-cont">
									<h4 class="tit">원하시는 이미지을 선택해주세요.</h4>
									<div class="layout-wrap">
										<div class="post-list-util">
											<div class="post-sorting" id="writeTab">
												
												<span><button type="button" class="btn on"
														write-type="profileList">내사진</button></span>
											</div>
										</div>

										<div class="kind-list postContents" id="stillList" style="display: none;">
											<!-- <ul></ul> -->
										</div>

										<div class="kind-list postContents" style="display: none;"
											id="trailerList">
											<!-- <ul></ul> -->
										</div>

										<div class="add-userfile postContents" 
											id="profileList">
											<div class="img-add">
											...
											<input id="img_select" name="fileName" type="file"  class="btn-image-add" accept="image/*" required  />
											<!-- <input type="hidden" name="fileNo" id="fileNo" value=""/> -->
										<!-- 		<button type="button" class="button purple"
													id="myphotoAddBtn">등록</button> -->
								...
								<div class="right-cont">
									...
									<div class="layout-wrap">
										<div class="post-wrap" id="postLayer">
											 ...
											 <div class="select on">
												<div class="post-layer add-post">
													<div class="post-bg">
														<img src="${path }/resources/img/bg-img-select.png"
															id="preview" alt="이미지를 선택하세요!">
													</div>
													<div class="post-txt">
													
														<textarea name="content" id="" title="내용을 작성해 주세요!"
															placeholder="내용을 작성해 주세요!"></textarea>
														<div class="text-count">0 / 50</div>
													</div>
													<div class="cancle-select">
														<a href="#" title="닫기" class="layer-close"><!-- <i
															class="iconset ico-layer-close"></i> --></a>
													</div>
												</div>
											</div>
											<div class="btn-group a-c pt30" id="postBtnLayer">
												...
												<!-- <button type="button" class="button dash-btn"
													onclick="fn_addPost();">
							...
							<div class="btn-group mt20">
								<!-- <a href="#" class="button large purple"title="등록"> id="regBtn" 
									등록 </a> <a href="#" class="button large" id="postCancelBtn"
									title="취소">취소</a> -->
									<input class="button large purple" type=submit value='글쓰기' />
									<a id="cancelbtn" class="button large">취소</a>
							</div>
