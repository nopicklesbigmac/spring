					...
					<div class="inner-wrap">
						<div class="tab-list fixed">
							<ul>
								...
								<li class="on"><a href="movie-post?rpstMovieNo=${movie.m_num }"
						...
						<div class="pb100">
							...
							<div class="movie-detail-poster bg-white">
								<div class="tit-util">
									<h2 class="tit small">
										${movie.m_name }에 대한<span class="font-gblue" id="postTotalCnt">${fn:length(post) }건</span>의
										무비포스트가 있어요!
									</h2>

									<a href="moviepost" id="goMoviePost" title="무비포스트 페이지로 이동" class="more"
										data-nm="탑건: 매버릭">더보기 <i
								...
								<!-- <div class="box">
									...
									<div class="poster-write">
										<a href="#tooltip-layer03" title="무비포스트 쓰기"
											class="tooltip-click"><i class="iconset ico-story-write"></i>
											무비포스트 쓰기</a>
										<div id="tooltip-layer03" class="tooltip-cont">
							...
							<div class="board-list-util">
								<!--     <p class="result-count" id="purplek"><strong>전체 (<b class="resultCountCnt">20</b>건)</strong></p> -->
								<p class="result-count" id="postTotCnt">
									<strong>전체 (<b class="resultCountCnt">${fn:length(post) }</b>건)
									</strong>
								</p>
								<!-- <a href="#" class="button moviePostWitBtn" title="무비포스트 쓰기">무비포스트 쓰기</a> -->
							</div>

							<!-- movie-post-list -->
							<div class="movie-post-list" id="moviePostList"
								style="position: relative; height: 1333.08px;">
								...
								<div class="wrap">
									<div class="img">
										...
										<a id="aclick" title="무비포스트 상세보기" 
											...
											h-data="auto" post-no="${post.p_num}"
											movie-no="${post.m_num}" data-row="${status.count}"
											post-date="${post.p_time }" alt="${movie.m_name}"data-id="${post.u_id }"
											data-url="${path }/resources/img/upload/${post.p_image }">
											<img src="${path }/resources/img/upload/${post.p_image }"
											alt="${movie.m_name}"> /&gt;
											<div style="display: none;">${post.p_content }</div>
										</a>
									</div>
									<div class="cont">
										<div class="writer">
											<a href="#" title="${post.u_id }님의 무비포스트 보기" class="moviePostId"
												data-id="${post.u_id }">${post.u_id }</a>
										</div>

										<!--href="movie-detail?rpstMovieNo=${post.m_num}"  -->
										<a 
											title="${movie.m_name} 상세보기"> <%-- --%>
											<p class="tit">${movie.m_name}</p>
										</a> 
										 <a id="aclick" title="무비포스트 상세보기"
											class="link btn-modal-open2 post-detailPopup" w-data="850"
											h-data="auto" post-no="${post.p_num}"data-id="${post.u_id }"
											movie-no="${post.m_num}" data-row="${status.count}"
											post-date="${post.p_time }" alt="${movie.m_name}"
											data-url="${path }/resources/img/upload/${post.p_image }">
											<div style="display: none;">${post.p_content }</div>
											<p class="txt">${post.p_content }</p>
											<p class="time">${post.p_time }</p>
										</a>
										<div class="condition">
											<button type="button"
												class="btn-like postLikeBtn listBtn jsMake" data-no="${post.m_num}">
												<i class="iconset ico-like">좋아요 수</i> <span class="none">${post.p_like}</span>
											</button>
											<a href="#layer_post_detail" title="댓글 작성하기"
												class="link btn-modal-open2 post-detailRlyPopup"
												w-data="850" h-data="auto" data-no="${list.m_num}" data-row="1"
												data-tot="192613"><i class="iconset ico-reply">댓글 수</i>
												0</a>
										</div>
									</div>
								</div>
							...
							<div class="more-movie-list" style="">
...
function fn_setMetaTag( detailInfo, moviePostNo, metaTagImg ){
	...
	if( detailPopup != null && detailPopup.backScreen != '' )	url  = '/moviepost/all' + attr;
 	...
 	settingMeta(metaFormat);
						...
						<!--################# 무비포스트 상세 레이어팝업으로 변경 ###########################  -->
						<section id="layer_post_detail" class="modal-layer2" style="z-index: 501;">
				<a href="" class="focus">레이어로 포커스 이동 됨</a>
				<div class="wrap"
