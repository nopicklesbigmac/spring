...
function fn_creditPay(){
	...
	IMP.request_pay({
		...
		if(rsp.success){
			...
			var param = {
				u_num:num,                    //회원 번호
				r_start:'${allDto.r_start}',    //영화 시작시간
				r_end:'${allDto.r_end}',        //영화 끝나는시간
				total_fee:'${allDto.total_fee}',  //예매 가격
				s_seat:'${allDto.s_seat}',     //예매 좌석
				total_seat_cnt:'${allDto.total_seat_cnt}',  //예매한 총 좌석수
				c_num:'${allDto.c_num}',    //영화관 번호
				s_num:'${allDto.s_num}',     //영화 상영관 번호
				m_num:'${allDto.m_num}',    //영화 번호
				r_num:'${allDto.r_num}'      //상영 번호
			};
			...
			<div class="seat-result">
				<div class="wrap">
					...
					<div class="tit-area type2">
						...
						<p class="date"><span id="playDe">${allDto.r_date }</span><em id="dowNm"></em> <span class="time" id="playTime"><i class="iconset ico-clock-white"></i>${allDto.r_start }~${allDto.r_end }</span></p>
					</div>
					<div class="price-process">
						<div class="box">
								<c:if test="${allDto.adult_cnt != 0}">
									<div class="data adult">
										<span class="tit">성인 <em>${allDto.adult_cnt }</em></span><span
											class="price">${allDto.adult_cnt*14000 }</span>
									</div>
								</c:if>
								
								<c:if test="${allDto.teenager_cnt != 0}">
									<div class="data teen">
