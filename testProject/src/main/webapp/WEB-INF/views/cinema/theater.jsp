...
<script src="${path}/resources/js/megabox-brch.js"></script>
...
function fn_bokdReload(page){
	...
	function fn_callSchedule(playDe) {
		...
		paramData.brchNo     = '${cinema.c_num}';
		paramData.brchNm     = '${cinema.c_name}';
	...
	function setMetaTagNNavi(index) {
		...
		var paramData = { 'scnTitle'     : '(${cinema.c_name})' + tabNm + ' > 전체극장 , 메가박스'
						, 'metaTagTitle' : '(${cinema.c_name})' + tabNm + ' > 전체극장 , 메가박스'
						, 'metaTagImg'   : 'http://image2.megabox.co.kr/mop/event.html/2018/6F/03D492-6FD4-485F-AE3F-2D3E2A41046F.jpg'};

		switch (index) {
		case 0 :
			paramData.metaTagUrl    = location.protocol + '//' + document.domain + '/theater?brchNo=${cinema.c_num}';
			paramData.metaTagMenuId = 'ON00000038';
			paramData.metaTagDtls   = '반갑습니다. 메가박스 ${cinema.c_name}점 입니다.';
			break;
		case 1 :
			paramData.metaTagUrl    = location.protocol + '//' + document.domain + '/theater/time?brchNo=${cinema.c_num}';
			paramData.metaTagMenuId = 'ON00000037';
			paramData.metaTagDtls   = '반갑습니다. 메가박스 ${cinema.c_name}점 입니다.';
			break;
		case 2 :
			paramData.metaTagUrl    = location.protocol + '//' + document.domain + '/theater/price?brchNo=${cinema.c_num}';
			paramData.metaTagMenuId = 'ON00000039';
			paramData.metaTagDtls   = '메가박스 메가박스 ${cinema.c_name}점 입니다.';
			break;
		}
				...
				<div class="inner-wrap">
					<div class="location">
						...
						<a href="/theater/list" title="전체극장 페이지로 이동">전체극장</a> <a
							href="https://www.megabox.co.kr/theater?brchNo=${cinema.c_num}
							title="극장정보페이지로이동">극장정보</a>
					</div>
				</div>
			</div>
			<!-- contents -->
							...
							<ul class="area-depth1">
									...
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num < 20 }">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
											</c:forEach>
										</ul>
									</div></li>
								<li><a href="" class="depth1" title="경기 선택">경기</a>
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num >= 20 and list.c_num <= 50 }">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
								...
								<li><a href="" class="depth1" title="인천 선택">인천</a>
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num >= 51 and list.c_num <= 55 }">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
								...
								<li><a href="" class="depth1" title="대전/충청/세종 선택">대전/충청/세종</a>
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num >= 56  and list.c_num <= 72}">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
								...
								<li><a href="" class="depth1" title="부산/대구/경상 선택">부산/대구/경상</a>
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num >= 73 and list.c_num <= 94  }">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
								...
								<li><a href="" class="depth1" title="광주/전라 선택">광주/전라</a>
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num >= 95 and list.c_num <= 103 }">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
								...
								<li><a href="" class="depth1" title="강원 선택">강원</a>
									<div class="area-depth2">
										<ul>
											<c:forEach var="list" items="${list }">
												<c:if test="${list.c_num >= 104 and list.c_num <= 108 }">
													<li data-brch-no="${list.c_num}"><a
														href="/root/theater?brchNo=${list.c_num}"
														title="${list.c_name} 상세보기">${list.c_name}</a></li>
												</c:if>
						...
						<div id="tab01" class="tab-cont on">
							...
							<div class="theater-facility">
								<c:forEach var="facility" items="${facility }">
									<p>
										<c:if test="${facility.f_facility == '일반상영관'}">
											<i class="iconset ico-facility-theater"></i>일반상영관
										</c:if>
										<c:if test="${facility.f_facility == '장애인석'}">
											<i class="iconset ico-facility-disabled"></i>장애인석
										</c:if>
										<c:if test="${facility.f_facility == '컴포트'}">
											<i class="iconset ico-facility-comfort"></i>컴포트
										</c:if>
										<c:if test="${facility.f_facility == 'MX'}">
											<i class="iconset ico-facility-mx"></i>MX
										</c:if>
										<c:if test="${facility.f_facility == '발코니'}">
											<i class="iconset ico-facility-balcony"></i>발코니
										</c:if>
										<c:if test="${facility.f_facility == '스위트룸'}">
											<i class="iconset ico-facility-sweet"></i>스위트룸
										</c:if>
										<c:if test="${facility.f_facility == '부티크'}">
											<i class="iconset ico-facility-boutique"></i>부티크
										</c:if>
										<c:if test="${facility.f_facility == '4인소파석'}">
											<i class="iconset ico-facility-fourpulson"></i>4인소파석
										</c:if>
										<c:if test="${facility.f_facility == '2인소파석'}">
											<i class="iconset ico-facility-twopulson"></i>2인소파석
										</c:if>
										<c:if test="${facility.f_facility == '스페셜석'}">
											<i class="iconset ico-facility-special"></i>스페셜석
										</c:if>
										<c:if test="${facility.f_facility == '돌비'}">
											<i class="iconset ico-facility-dolby"></i>돌비
										</c:if>
										<c:if test="${facility.f_facility == '커플석'}">
											<i class="iconset ico-facility-couple"></i>커플석
										</c:if>
										<c:if test="${facility.f_facility == '로얄석'}">
											<i class="iconset ico-facility-royal"></i>로얄석
										</c:if>
										<c:if test="${facility.f_facility == '테이블존'}">
											<i class="iconset ico-facility-table"></i>테이블존
										</c:if>
										<c:if test="${facility.f_facility == '키즈석'}">
											<i class="iconset ico-facility-kids"></i>키즈석
										</c:if>
									</p>
								</c:forEach>
							</div>
							...
							<ul class="dot-list">
								<c:forEach var="floor" items="${floor }">
									<script>
								var str ='${floor.f_floor }';
								var fff = str.split(':');
								</script>
									<li><span class="font-gblue"> <script> document.write(fff[0]+':');</script>
									</span> <script>document.write(fff[1]);</script></li>
								</c:forEach>
							</ul>
							...
							<ul class="dot-list">
								<li><span class="font-gblue">도로명주소 : </span>
									${cinema.c_state} ${cinema.c_city} ${cinema.c_detail}</li>
							...
							<div class="location-map-btn mt15">
								<div class="btn-group left">
									<a
										href="http://m.map.naver.com/map.nhn?lng=127.12535&amp;lat=37.528423&amp;level=2"
								...
								<div class="parking-section">
									...
									<div class="info">
										...
										<ul class="dot-list">
											...
											<li>인근 도로는 주차 금지구역으로 주차 시 과태료가 부과될 수 있사오니 인근 유료 주차장을 이용해
												주시기 바랍니다.</li>
								...
								<div class="transportation-section">
									...
									<div class="info">
										...
										<ul class="dot-list">
											...
											<li>3319번, 3321번, 3220번, 3212번, 3214번, 3411번, 3318번,
												3321번</li>
							...
							<div id="brchNoti">
								...
								<div class="accordion-list">
									<ul>
										<li><div class="title ">
												<a href="" title="[강동] 관람요금 변경 안내">
													<div class="cont-tit">[강동] 관람요금 변경 안내</div>
													<p class="cont-admin">강동</p>
													<p class="cont-date">2022.06.24</p>
												</a>
											</div>
											<div class="content" style="display: none">
												안녕하세요.<br> 메가박스 강동지점입니다.<br> <br> 아래와 같이 영화
												요금제가 변경되오니, 이용에 참고 부탁드립니다.<br> <br> 1. 적용 일시 :
												2022년 7월 4일(월) 상영작부터<br> 2. 주요 사항 : 조조/주중/주말 성인, 청소년 기존
												가격에서 1,000원씩 인상<br> <br> 자세한 요금 안내는 7월 4일 월요일 이후
												지점 안내 페이지를<br> 확인해주시기 바랍니다.<br> ※ 확인 방법 : 홈페이지
												&gt; 극장 &gt; 지점 &gt; 관람료<br> <br> 감사합니다.
											</div></li>
									</ul>
								</div>
								<!--// accordion-list -->
	...
	<div class="tooltip-layer"
		style="display: block; min-width: 200px; z-index: 501; opacity: 0;">
